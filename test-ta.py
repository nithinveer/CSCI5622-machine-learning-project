...
def run(url):
    ...
    for p in range(2, pageNum):
        ...
        if p == 1:
            ...
            print
            "hello"
            pageLink = 'https://www.tripadvisor.com/RestaurantSearch-g60713-oa' + str(
                p * 30) + '-San_Francisco_California.html#EATERY_OVERVIEW_BOX'
        ...
        for review in reviews:
            ...
            rev = rev.strip()
            f = open('TripAdv''\\' + str(rev) + '.txt',
                     'w')  # Opening a file by the name of the Restaurant to save the reviews
            ...
            page = revsoup.findAll('a', {
                'class': 'pageNum taLnk'})  # Finding page numbers till which the loop has to be run to scrape reviews and ratings
            for p1 in page:
                finalPage = p1.contents[0]  # FinalPage gives the page number
            print
            finalPage
            for p2 in range(0,
                            int(finalPage)):  # Loop for finding reviews,Rating and Date for each page in a restaurant
                ...
                if p2 == 0:
                    ...
                    for r in revs:
                        ...
                        try:
                            ...
                            print
                            ""
                        link_rev = 'https://www.tripadvisor.com/' + r.get(
                            'href')  # Creating link to go inside teh review
                        # print link_rev
                        for q in range(0, 5):
                            try:
                                response = requests.get(link_rev, headers={
                                    'User-Agent': 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36', })
                        ...
                        rating = reviewPage.findAll('img', {'class': re.compile(
                            'sprite-rating_s_fill rating_s_fill s')})  # Scrapping rating from the review
                    ...
                    print
                    last
                    finalLink = first[0] + 'Reviews-or' + str(p2 * 10) + last[
                        0] + 'REVIEWS'  # To create link to Navigate in review pages
                    for q in range(0, 5):
                        try:
                            response = requests.get(finalLink, headers={
                                'User-Agent': 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36', })
                            Webcontent_rev = response.content
                            break
                        except Exception as e:
                    ...
                    for r in revs:
                        ...
                        for q in range(0, 5):
                            try:
                                ...
                                ss = input("No internet: Enter Anything")
                        reviewPage = BeautifulSoup(Webcontent_rev)
                        rating = reviewPage.findAll('img',
                                                    {'class': re.compile('sprite-rating_s_fill rating_s_fill s')})
                        reviewRating = rating[0].get('alt')
                        # reviewRating=reviewRating[0]
                        print
                        reviewRating
